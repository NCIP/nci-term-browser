 ...
 * <p>
 * Note: the match algorithm applied to the term is the standard
 * lucene query syntax.
 */
public class FindRelatedNodesForTermAndAssoc {
	...
	public static void main(String[] args){
		if (args.length < 2) {
			System.out.println(
			"Example: FindRelatedNodesForTermAndAssoc \"lung\" \"Anatomic_Structure_Is_Physical_Part_Of\"");
			return;
		};
...
*/
	 ...
	 * @throws LBException
	 */
	protected void printTargets(String term, String assoc, EVSApplicationService lbSvc, String scheme, CodingSchemeVersionOrTag csvt)
	throws LBException
	{
		// Find all nodes that the term matches.
		/*
			...
			if (matchCount > 0) {
				...
				for (int j = 0; j < matchCount; j++) {
					...
					for (int k = 0; k < acl.length; k++) {
						...
						EntityDescription ed = ac.getEntityDescription();
						Util.displayMessage(
							'\t' + (aName == null ? a.getAssociationName() : aName)
								+ ": " + ac.getConceptCode() + '/'
									+ (ed == null ? "**No Description**" : ed.getContent()));
					}
				}
