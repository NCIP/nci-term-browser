...
<f:view>
 ...
 %>
  ...
  <%
    if (menubar_version == null) {
  %> <a
    href="<%=request.getContextPath()%>/pages/vocabulary_home.jsf?dictionary=<%=dictionary%>&scheme=<%=menubar_scheme%>">Home</a>
  <%
    } else {
  %> <a
    href="<%=request.getContextPath()%>/pages/vocabulary_home.jsf?dictionary=<%=dictionary%>&scheme=<%=menubar_scheme%>&version=<%=menubar_version%>">Home</a>
  <%
    }
 ...
 %> | <a href="<%=request.getContextPath()%>/pages/subset.jsf">Subsets</a>
  ...
  %> | <a href="<%=request.getContextPath()%>/pages/help.jsf">Help</a></div>
    ...
    <%
      int n1 = 0;
            for (int i = 0; i < v.size(); i++) {
              ...
              String meta_prop_name = (String) ret_vec
                  .elementAt(0);
