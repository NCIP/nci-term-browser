  ...
  <table>
    <tr>
      ...
      <td class="textbody" align=right>
        ...
        &nbsp;
        <i>
          <a href="<%=request.getContextPath() %>/pages/value_set_entity_search_results.jsf?page_number=<%=prev_page_num_str%>">Prev</a>
        </i>&nbsp;
        <%
          }
          if (num_pages > 1) {
		 ...
		 for (int idx=sliding_window_start; idx<=sliding_window_end; idx++) { 
		    ...
		    if (page_num != idx) {
		      %>
			<a href="<%=request.getContextPath() %>/pages/value_set_entity_search_results.jsf?page_number=<%=idx_str%>"><%=idx_str%></a>
			&nbsp;
		      <%
		    } else {
		      %>
        ...
        %>
          &nbsp;
          <i>
            <a href="<%=request.getContextPath() %>/pages/value_set_entity_search_results.jsf?page_number=<%=next_page_num_str%>">Next</a>
          </i>
        <%
          } 
